    ...
    public function show(Request $request, Contract $contract): Response
    {
        ...
        return Inertia::render('Contracts/Show', [
            ...
            'can' => [
                ...
                'create_variation' => $request->user()->can('contract.variation.create') && $contract->canManageVariations(),
                'create_invoice' => $request->user()->can('contract.invoice.create') && $contract->canManageInvoices(),
                'approve_invoice' => $request->user()->can('contract.invoice.approve') && $contract->canManageInvoices(),
                'pay_invoice' => $request->user()->can('contract.invoice.pay') && $contract->canManageInvoices(),
                'approve_variation' => $request->user()->can('contract.variation.approve') && $contract->canManageVariations(),
                ...
                'initialize_closeout' => $request->user()->can('update', $contract) && $contract->canInitializeCloseout(),
